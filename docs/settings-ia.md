...
- **Appearance** — every visual surface has one destination here.
  ...
  - `Status Bar [View]` — the Projmux-owned status segments. `Notifications
    ...
    explicit HUD windows, `5h` and `Weekly`; Claude additionally owns a `Model`
    Toggle for the runtime model name its hooks report (default on, first
    element shed on a narrow row); Antigravity has no usage source and
    no provider View. Parent off states gate effective visibility without rewriting
    saved child values. Provider/window/model rows show saved, effective, and source.
